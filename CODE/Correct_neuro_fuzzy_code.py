...
y_train = data_train['current yr TMRF']
...
TMRF = ctrl.Consequent(np.arange(0, 1.1, 0.1), 'current yr TMRF')
...
# Define membership functions for TMRF
TMRF['low'] = fuzz.trimf(TMRF.universe, [0, 0, 0.5])
TMRF['medium'] = fuzz.trimf(TMRF.universe, [0, 0.5, 1])
TMRF['high'] = fuzz.trimf(TMRF.universe, [0.5, 1, 1])

# Define fuzzy rules
rule1 = ctrl.Rule(N_HMAX['low'] & N_MMAX['low'] & N_LMIN['low'] & N_SLP['low'] & N_RH['low'], TMRF['low'])
rule2 = ctrl.Rule(N_HMAX['medium'] & N_MMAX['medium'] & N_LMIN['medium'] & N_SLP['medium'] & N_RH['medium'], TMRF['medium'])
rule3 = ctrl.Rule(N_HMAX['high'] & N_MMAX['high'] & N_LMIN['high'] & N_SLP['high'] & N_RH['high'], TMRF['high'])
...
def compute_fuzzy_outputs(X):
    ...
    for row in X:
        ...
        fuzzy_sim.compute()
        fuzzy_outputs.append([fuzzy_sim.output['current yr TMRF']])
